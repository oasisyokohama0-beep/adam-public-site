...
## 2026-06-26 - お客様の声ページ拡充
- `/reviews` の見せ方を更新。評価サマリー、注目口コミ、口コミカード、投稿導線、空状態を追加。
- 参考サイトは構成の参考のみ。口コミ本文はコピーせず、ADAM TOKYO 用の掲載サンプルとして作成。
- `Review` 型は変更なし。`lib/data/reviews.json` の既存フィールドのみを使用し、井口さん側の Supabase 置換境界を維持。
